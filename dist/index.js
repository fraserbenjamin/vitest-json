...
function toMatchJSON(received, expected) {
    const { matcherErrorMessage, RECEIVED_COLOR, printDiffOrStringify, printExpected, printReceived, stringify, } = this.utils;
    const hint = this.utils.matcherHint("toMatchJSON", undefined, undefined, {
        isNot: this.isNot,
        promise: this.promise,
    });
    if (typeof received !== "string") {
        throwError(matcherErrorMessage(hint, `${RECEIVED_COLOR("received")} value must be a valid JSON string`));
    }
    try {
        ...
        throwError(matcherErrorMessage(hint, `${RECEIVED_COLOR("received")} value must be a valid JSON string. ${message}`, isEmpty
...
vitest_1.expect.extend({ jsonMatching, toMatchJSON });
/**
